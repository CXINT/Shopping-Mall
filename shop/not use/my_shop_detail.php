...
<div class="nav">
    <div class="notice">
        <a href="http://i.jumei.com/i/account/avatar" target="_blank" style="border-bottom: 0;padding:0;"></a>
        <p class="nickname"><a style="padding-left:0;line-height: 1.8;color:#ed145b" href="" target="_blank"></a></p>
        <p class="nickname"><a style="padding-left:0;line-height: 1.8;color:#ed145b" href="" target="_blank"><?=$_SESSION['homeInfo']['username']?></a></p>
        <p class="uid">用户ID: <?=$_SESSION['homeInfo']['id']?></p>
    </div>
    <h2><b></b>我的聚美优品</h2>
    <a href="http://i.jumei.com/i/order/list" class="list selected"><b></b>我的订单</a>
    
    <h2><b></b>管理个人账户</h2>
    <a href="./personinfo.php" class="share"><b></b>管理个人账户</a>
</div>
...
<h1>
    <a href="http://hd.jumei.com/act/9-478-1477-download_app.html" target="_blank">手机客户端，随时随地查看物流详情</a>
    我的订单
</h1>
...
@include '../Public/config.php';
      ...
      $sql = 'select id from '.FIX."orders where uid = $id";
      $res = mysqli_query($link, $sql);
        if ($res && mysqli_num_rows($res) > 0){
          while ($row = mysqli_fetch_assoc($res)){
            echo "<p>交易单号：{$row['id']}</p>";               
}
}
 ?> 
...
@include '../Public/config.php';
      $link = mysqli_connect(HOST, USER, PWD, DB) or die('数据库连接失败');
      mysqli_set_charset($link, 'utf8');
      $sql2 = 'select total from '.FIX."orders where uid = $id";
      // var_dump($sql2);
        $res2 = mysqli_query($link, $sql2);
            if($res2 && mysqli_num_rows($res2) > 0){
        while($row2 = mysqli_fetch_assoc($res2)){
            echo "<p>商品金额: ¥{$row2['total']}(免运费)</p>";
        }
}
?>
</td> -->
...
@include '../Public/config.php';
      $link = mysqli_connect(HOST, USER, PWD, DB) or die('数据库连接失败');
      mysqli_set_charset($link, 'utf8');
      $sql5 = "select name from shop_goods where id in(select gid from shop_detail where oid in(select id from shop_orders where uid = $id))";
...
@include '../Public/config.php';
      $link = mysqli_connect(HOST, USER, PWD, DB) or die('数据库连接失败');
      mysqli_set_charset($link, 'utf8');
      $sql5 = 'select pic from shop_goods where id in(select gid from shop_detail where oid in(select id from shop_orders where uid = $id))';
        $res5 = mysqli_query($link, $sql5);
            if($res5 && mysqli_num_rows($res5) > 0){
        while($row5 = mysqli_fetch_assoc($res5)){
            echo "<img src='{$row5['pic']}' alt='无印良品滋润型 水乳套组'>";
}        
}        
?>
            </td>-->      
...
@include '../Public/config.php';
      $link = mysqli_connect(HOST, USER, PWD, DB) or die('数据库连接失败');
      mysqli_set_charset($link, 'utf8');
      $sql3 = 'select num from '.FIX."detail where oid in(select id from shop_orders where uid = $id)";
      // var_dump($sql2);
...
@include '../Public/config.php';
      $link = mysqli_connect(HOST, USER, PWD, DB) or die('数据库连接失败');
      mysqli_set_charset($link, 'utf8');
      $sql4 = "select price from shop_goods where id in(select gid from shop_detail where oid in(select id from shop_orders where uid = $id))";
...
@include '../Public/config.php';
      $link = mysqli_connect(HOST, USER, PWD, DB) or die('数据库连接失败');
      mysqli_set_charset($link, 'utf8');
      $sql ="select shop_orders.id,shop_orders.total,shop_orders.addtime,shop_orders.uid,shop_orders.status, shop_detail.oid,shop_detail.gid,shop_detail.num, shop_goods.id,shop_goods.name,shop_goods.price,shop_goods.pic from shop_orders,shop_detail,shop_goods where shop_orders.id=shop_detail.oid and shop_detail.gid=shop_goods.id";
      $res = mysqli_query($link, $sql);
        if ($res && mysqli_num_rows($res) > 0) {
         while ($row = mysqli_fetch_assoc($res)) {
            $status = ($row['status']==1)?'已付款,待收货':'确认收货';
                echo "<tr align='center'>
            <td><span>订单编号:{$row['oid']}<span><br />
                <span>商品总额:¥{$row['total']}</span>
            </td>
            ...
            <td>
                <a href='action.php?a=delDetail'>删除</a>
            </td>
        </tr>";
    }
} else {
...
<!--<a href='edit.php?id={$row['id']}&oid={$row['oid']}'>编辑</a> -->       
        ...
        <tr class="separator">
            <td colspan="7"></td>                                             
        <tr class="separator">                            
            <td colspan="7" style="border-bottom-color: white;"></td>
        </tr>
        </tbody></table>
        <table id="table-header-fixed" style="display: none;">
        <tbody>
        <tr class="order_list_title">
        ...
        <a href="" class="backtotop" style="bottom: 100px;"><b></b>回到顶部</a>
<style type="text/css">
    .order_list_title {
